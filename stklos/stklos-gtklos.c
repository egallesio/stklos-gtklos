/*
 * stklos-gtklos.c		-- Various GTk+ wrappers for GTklos 
 * 
 ...
 * Last file update: 19-Sep-2009 18:10 (eg)
 */
...
static SCM GValue2Scheme(GValue *value, SCM prop)
{
  ...
  switch (G_VALUE_TYPE(value)) {
    case G_TYPE_NONE: 
      res = STk_void; break;
    case G_TYPE_CHAR: 
      res = MAKE_CHARACTER((unsigned char) g_value_get_char(value)); break;
    case G_TYPE_UCHAR: 
...
static void assign_GValue(GValue *value, SCM prop, SCM val)
{
  ...
  */
  switch (G_VALUE_TYPE(value)) {
    ...
    case G_TYPE_CHAR: 
      if (CHARACTERP(val))
	g_value_set_char(value, (char) CHARACTER_VAL(val));
      else 
 ...
 * 	%gtk-get-size ...
 * ---------------------------------------------------------------------- */
DEFINE_PRIMITIVE("%gtk-get-size", gtk_get_size, subr1, (SCM obj))
{
  gint width, height;
  
  if (!CPOINTERP(obj)) error_bad_widget(obj);

  if (GTK_WIDGET_DRAWABLE(CPOINTER_VALUE(obj))) 
    gdk_window_get_size(GTK_WIDGET(CPOINTER_VALUE(obj))->window, &width, &height);
  else 
    width = height = -1;

  return STk_cons(MAKE_INT(width), MAKE_INT(height));
}
...
DEFINE_PRIMITIVE("%container-children", cont_children, subr1, (SCM w))
{
  ...
  gl = gtk_container_children(GTK_CONTAINER(CPOINTER_VALUE(w)));
...
DEFINE_PRIMITIVE("%string->color", string2color, subr1, (SCM str))
{
  ...
  color = STk_must_malloc(sizeof(GdkColor));
  if (!gdk_color_parse(STRING_CHARS(str), color))
...
DEFINE_PRIMITIVE("%color->string", color2string, subr1, (SCM obj))
{
  ...
  return STk_Cstring2string(gdk_color_to_string(CPOINTER_VALUE(obj)));
}
...
MODULE_ENTRY_START("stklos-gtklos") {
  ...
  ADD_PRIMITIVE_IN_MODULE(gtk_get_size, gtklos_module);
